...
#   XUI_PASS      面板密码          默认随机 10 位
#   XUI_PORT      面板端口          默认随机 10000-65535
#   XUI_PATH      面板根路径        默认随机 6 位
...
curl -L -o /usr/local/x-ui-linux-${cpu}.tar.gz -# --retry 2 --insecure ${REL_BASE}/x-ui-linux-${cpu}.tar.gz
...
curl -L -o /usr/bin/x-ui -# --retry 2 --insecure ${RAW_BASE}/install.sh
...
curl -sL ${RAW_BASE}/version | awk -F "更新内容" '{print $1}' | head -n 1 > /usr/local/x-ui/v
showxuiip
sleep 2
xuigo
cronxui
...
green "x-ui已卸载完成"
echo
blue "欢迎继续使用x-ui-yg脚本：bash <(curl -Ls ${RAW_BASE}/install.sh)"
exit
else
red "输入有误" && uninstall
fi
...
curl -L -o /usr/local/x-ui/xuiwpph -# --retry 2 --insecure ${RAW_BASE}/xuiwpph_$cpu
...
freeport(){
local p=$1
if [[ -z $p ]] || ss -tunlp 2>/dev/null | awk '{print $5}' | sed 's/.*://g' | grep -qw "$p"; then
p=$(shuf -i 10000-65535 -n 1)
while ss -tunlp 2>/dev/null | awk '{print $5}' | sed 's/.*://g' | grep -qw "$p"; do
p=$(shuf -i 10000-65535 -n 1)
done
fi
echo "$p"
...
# 方式一：直接给一份现成的配置（本地文件或 URL）
if [[ -n $XUI_TPL ]]; then
if [[ $XUI_TPL == http* ]]; then
curl -sL --retry 2 -o "$out" "$XUI_TPL" || return 1
else
cp -f "$XUI_TPL" "$out" || return 1
fi
jq empty "$out" >/dev/null 2>&1 || { red "自定义模板不是合法 JSON"; return 1; }
return 0
fi

# 方式二：按 XUI_PROXY 生成一份带出站代理的模板
[[ -z $XUI_PROXY ]] && return 1

local body=${XUI_PROXY#*://}
local hostport userpass phost pport puser ppass srv
if [[ $body == *@* ]]; then
userpass=${body%@*}
hostport=${body##*@}
puser=${userpass%%:*}
...
phost=${hostport%:*}
pport=${hostport##*:}
if [[ "$hostport" != *:* || -z $phost || -z $pport ]]; then
red "XUI_PROXY 缺少主机或端口，应为 socks5://user:pass@host:port"
return 1
fi
...
cat > "$out" <<EOF
{
  ...
  "inbounds": [
    { "listen": "127.0.0.1", "port": 62789, "protocol": "dokodemo-door",
  ...
  "outbounds": [
    ...
    { "tag": "vps-outbound-v4", "protocol": "freedom", "settings": { "domainStrategy": "UseIPv4v6" } },
    { "tag": "vps-outbound-v6", "protocol": "freedom", "settings": { "domainStrategy": "UseIPv6v4" } },
    { "tag": "socks5-warp", "protocol": "socks",
      "settings": { "servers": [ { "address": "127.0.0.1", "port": 40000 } ] } },
    { "tag": "socks-proxy", "protocol": "socks",
      "settings": { "servers": [ $srv ] } },
    { "tag": "socks5-warp-v4", "protocol": "freedom",
      "settings": { "domainStrategy": "UseIPv4v6" }, "proxySettings": { "tag": "socks5-warp" } },
    { "tag": "socks5-warp-v6", "protocol": "freedom",
  ...
  "routing": {
    ...
    "rules": [
      ...
      { "type": "field", "domain": [ $dd ], "outboundTag": "direct" },
      { "type": "field", "domain": [ $pd ], "outboundTag": "socks-proxy" },
...
EOF
jq empty "$out" >/dev/null 2>&1 || { red "生成的模板不是合法 JSON"; return 1; }
return 0
}

# 把模板写进面板数据库，让它成为默认 Xray 配置
apply_tpl(){
local tpl=/tmp/xui_tpl_$$.json
gen_tpl "$tpl" || { yellow "未提供 XUI_TPL / XUI_PROXY，跳过自定义 Xray 配置"; rm -f "$tpl"; return 0; }

if ! command -v sqlite3 >/dev/null 2>&1; then
if [ -x "$(command -v apt-get)" ]; then apt-get install -y sqlite3 >/dev/null 2>&1
elif [ -x "$(command -v yum)" ]; then yum install -y sqlite >/dev/null 2>&1
elif [ -x "$(command -v dnf)" ]; then dnf install -y sqlite >/dev/null 2>&1
elif [ -x "$(command -v apk)" ]; then apk add sqlite >/dev/null 2>&1
fi
fi
if ! command -v sqlite3 >/dev/null 2>&1; then
red "缺少 sqlite3，无法写入默认配置。装完后请手动在面板里粘贴 Xray 配置"
rm -f "$tpl"; return 1
fi

local db
db=$(xui_db) || { red "找不到面板数据库"; rm -f "$tpl"; return 1; }

# 表名在不同分支里可能是 settings 或 setting
local tbl=''
...
[[ -z $tbl ]] && { red "数据库里找不到设置表"; rm -f "$tpl"; return 1; }

if [[ x"${release}" == x"alpine" ]]; then rc-service x-ui stop >/dev/null 2>&1; else systemctl stop x-ui >/dev/null 2>&1; fi
sleep 1

if sqlite3 "$db" "SELECT 1 FROM $tbl WHERE key='xrayTemplateConfig' LIMIT 1;" 2>/dev/null | grep -q 1; then
sqlite3 "$db" "UPDATE $tbl SET value=readfile('$tpl') WHERE key='xrayTemplateConfig';" 2>/dev/null
else
sqlite3 "$db" "INSERT INTO $tbl (key,value) VALUES ('xrayTemplateConfig', readfile('$tpl'));" 2>/dev/null
fi
local rc=$?

if [[ x"${release}" == x"alpine" ]]; then rc-service x-ui start >/dev/null 2>&1; else systemctl start x-ui >/dev/null 2>&1; fi
sleep 2
rm -f "$tpl"
...
open(p, "w", encoding="utf-8").write(json.dumps({
    ...
    "pool_size": 100, "mode": "round_robin", "sticky": "",
    "fail_n": 3, "check_interval": 120, "connect_timeout": 8,
    "fetch_url": "", "fetch_cmd": "", "auto_rotate": 30,
    "sc_count": 100, "sc_time": 0, "sc_protocol": "s5",
...
# 申请 IP 证书并开启 https 登录
# 等价于原流程：certinstall 输入 1 → acme 菜单 1 → 模式 1 → 邮箱回车 → IP 回车
auto_cert(){
if [[ ${XUI_HTTPS:-1} != 1 ]]; then
yellow "按设置跳过证书申请，使用 http 登录"
return 0
fi

# 已有证书直接复用
if [[ -s /root/ygkkkca/cert.crt && -s /root/ygkkkca/private.key ]]; then
/usr/local/x-ui/x-ui cert -webCert /root/ygkkkca/cert.crt -webCertKey /root/ygkkkca/private.key >/dev/null 2>&1
green "检测到已有证书，直接启用 https 登录"
return 0
fi

mkdir -p /root/ygkkkca

# 释放 80 端口（对应 acme-yg 的 acme2）
if ! command -v lsof >/dev/null 2>&1; then
if [ -x "$(command -v apt-get)" ]; then apt-get install -y lsof >/dev/null 2>&1
elif [ -x "$(command -v yum)" ]; then yum install -y lsof >/dev/null 2>&1
elif [ -x "$(command -v dnf)" ]; then dnf install -y lsof >/dev/null 2>&1
elif [ -x "$(command -v apk)" ]; then apk add lsof >/dev/null 2>&1
fi
fi
if command -v lsof >/dev/null 2>&1; then
lsof -i :80 2>/dev/null | grep -v PID | awk '{print $2}' | sort -u | xargs -r kill -9 >/dev/null 2>&1
elif command -v fuser >/dev/null 2>&1; then
fuser -k 80/tcp >/dev/null 2>&1
fi
sleep 1

# 邮箱：不给就随机虚拟 gmail（对应原流程回车跳过）
local Aemail=${XUI_EMAIL:-$(date +%s%N | md5sum | cut -c 1-6)@gmail.com}
yellow "证书注册邮箱：$Aemail"
...
curl -s https://get.acme.sh | sh -s email=$Aemail >/dev/null 2>&1
if [[ -z $(~/.acme.sh/acme.sh -v 2>/dev/null) ]]; then
red "acme.sh 安装失败，继续用 http 登录"
return 1
fi
bash ~/.acme.sh/acme.sh --upgrade --use-wget --auto-upgrade >/dev/null 2>&1

# 选 IP（对应 ACMEstandaloneIP 回车默认取本机 IP）
v4v6
local vpsip
[[ -z $v4 ]] && vpsip=$v6 || vpsip=$v4
local ym=${XUI_CERT_IP:-$vpsip}
if [[ -z $ym ]]; then
red "取不到本机公网 IP，跳过证书申请"
return 1
fi
if [[ "$ym" == 2a09* || "$ym" == 104.28* ]]; then
red "检测到 WARP IP，无法申请 IP 证书，继续用 http 登录"
return 1
fi
green "开始申请 IP 证书：$ym"
...
crontab /tmp/crontab.tmp >/dev/null 2>&1
rm -f /tmp/crontab.tmp
bash ~/.acme.sh/acme.sh --list 2>/dev/null | awk 'NR>1{print $1}' | tail -1 > /root/ygkkkca/ca.log
/usr/local/x-ui/x-ui cert -webCert /root/ygkkkca/cert.crt -webCertKey /root/ygkkkca/private.key >/dev/null 2>&1
green "IP 证书申请成功，已开启 https 登录"
return 0
else
red "IP 证书申请失败，继续用 http 登录"
rm -rf /root/ygkkkca
return 1
fi
}
...
local f
for f in 主程序.py 解析.py 池.py 转发.py 网页.py 更新.py 面板.html 登录.html 对接.md 配置.示例.json 最低消耗.json 写入分流.py xui-bridge.service; do
self_get "/tmp/xui-bridge/${f}" "vps桥/${f}" || {
red "拉桥文件失败 vps桥/${f}（私有仓库请设置 GH_TOKEN）"
return 1
}
done
cp -f /tmp/xui-bridge/主程序.py /tmp/xui-bridge/解析.py /tmp/xui-bridge/池.py /tmp/xui-bridge/转发.py /tmp/xui-bridge/网页.py /tmp/xui-bridge/更新.py /tmp/xui-bridge/面板.html /tmp/xui-bridge/登录.html /tmp/xui-bridge/对接.md /tmp/xui-bridge/写入分流.py /tmp/xui-bridge/最低消耗.json /opt/xui-bridge/
...
# 1. 防火墙
if [[ ${XUI_FIREWALL:-1} == 1 ]]; then close; else yellow "按设置跳过防火墙处理"; fi

# 2. 下载安装面板
serinstall

# 3. 账号密码（禁止含 admin，和原版一致）
username=${XUI_USER:-$(date +%s%N | md5sum | cut -c 1-6)}
password=${XUI_PASS:-$(date +%s%N | md5sum | cut -c 1-10)}
if [[ ${username} == *admin* ]]; then username=$(date +%s%N | md5sum | cut -c 1-6); yellow "用户名含 admin，已改为随机值"; fi
if [[ ${password} == *admin* ]]; then password=$(date +%s%N | md5sum | cut -c 1-10); yellow "密码含 admin，已改为随机值"; fi
/usr/local/x-ui/x-ui setting -username ${username} -password ${password} >/dev/null 2>&1

# 4. 端口
port=$(freeport "$XUI_PORT")
/usr/local/x-ui/x-ui setting -port $port >/dev/null 2>&1

# 5. 根路径
path=${XUI_PATH:-$(date +%s%N | md5sum | cut -c 1-6)}
/usr/local/x-ui/x-ui setting -webBasePath ${path} >/dev/null 2>&1

# 6. 申请 IP 证书，开启 https 登录
auto_cert

# 7. 自签证书（给 Hysteria2 用，和原版一致）
mkdir -p /root/ygkkkcaz
command -v openssl >/dev/null 2>&1 && openssl ecparam -genkey -name prime256v1 -out /root/ygkkkcaz/private.key >/dev/null 2>&1
command -v openssl >/dev/null 2>&1 && openssl req -new -x509 -days 36500 -key /root/ygkkkcaz/private.key -out /root/ygkkkcaz/cert.crt -subj "/CN=www.bing.com" >/dev/null 2>&1
...
latestV=$(curl -sL ${RAW_BASE}/version | awk -F "更新内容" '{print $1}' | head -n 1)
...
echo -e "${yellow}$(curl -sL ${RAW_BASE}/version)${plain}"
