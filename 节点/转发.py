...
async def 测通路(池子: 池) -> dict:
    """经本机 41000 去连 www.dola.com。池字节不增加就算失败。"""
    if not 池子.条们:
        return {"ok": False, "err": "池是空的，先在页面贴代理"}
    if not 池子.健康们() and not [一 for 一 in 池子.条们 if 一.启用]:
        return {"ok": False, "err": "没有启用的上游"}
    前 = sum(一.上行 + 一.下行 for 一 in 池子.条们)
    主 = str(池子.设.get("listen") or "127.0.0.1")
    口 = 池子.听口()
    目标 = str(池子.设.get("check_host") or "www.dola.com").strip() or "www.dola.com"
    秒 = float(池子.设.get("connect_timeout") or 8)
    读 = 写 = None
    try:
        读, 写 = await asyncio.wait_for(asyncio.open_connection(主, 口), timeout=秒)
        写.write(b"\x05\x01\x00")
        await 写.drain()
        答 = await asyncio.wait_for(读.readexactly(2), timeout=秒)
        if 答 != b"\x05\x00":
            return {"ok": False, "err": f"41000 不是本机桥 SOCKS（{答!r}）"}
        名 = _域名字节(目标)
        写.write(b"\x05\x01\x00\x03" + bytes([len(名)]) + 名 + struct.pack("!H", 443))
        await 写.drain()
        头 = await asyncio.wait_for(读.readexactly(4), timeout=秒)
        if 头[1] != 0:
            return {"ok": False, "err": f"经桥 CONNECT {目标}:443 失败 code={头[1]}"}
        atyp = 头[3]
        if atyp == 1:
            await 读.readexactly(6)
        elif atyp == 3:
            n = (await 读.readexactly(1))[0]
            await 读.readexactly(n + 2)
        elif atyp == 4:
            await 读.readexactly(18)
        if hasattr(写, "start_tls"):
            ctx = ssl.SSLContext(ssl.PROTOCOL_TLS_CLIENT)
            ctx.check_hostname = False
            ctx.verify_mode = ssl.CERT_NONE
            await asyncio.wait_for(写.start_tls(ctx, server_hostname=目标), timeout=秒)
        await asyncio.sleep(0.3)
    except Exception as 错:
        return {"ok": False, "err": f"断在桥或上游：{错}"}
    finally:
        if 写 is not None:
            try:
                写.close()
                ...
                pass
    后 = sum(一.上行 + 一.下行 for 一 in 池子.条们)
    连 = sum(一.连接 for 一 in 池子.条们)
    if 后 <= 前 and 连 <= 0:
        return {"ok": False, "err": "请求发出了，但池的连接/上下行没增加，桥没接到活"}
    return {
        "ok": True,
        "msg": f"通路通：{目标}:443 经 127.0.0.1:{口}，池字节 +{后 - 前}",
        "前": 前,
        "后": 后,
    }
