...
class 池:
    ...
    def 落盘(self) -> None:
        ...
        出 = {
            ...
            "node_port": int(self.设.get("node_port") or 14564),
            "node_uuid": str(self.设.get("node_uuid") or ""),
            "node_listen": str(self.设.get("node_listen") or "0.0.0.0"),
            "public_host": str(self.设.get("public_host") or ""),
    ...
    async def 改设(self, 补: dict[str, Any]) -> None:
        async with self.锁:
            for k in ("mode", "sticky", "fetch_url", "fetch_cmd", "node_uuid", "node_listen", "public_host"):
                if k in 补 and 补[k] is not None:
                    self.设[k] = 补[k]
            for k in ("pool_size", "fail_n", "check_interval", "check_conc", "connect_timeout", "node_port"):
