...
**一条命令装完。** 新机装面板+桥；已经有面板就只更新桥、打开公网管理页。桥分流默认关闭，X-UI 保持原设置，要走代理需在桥管理页手动打开。
...
桥默认每 5 分钟自己去仓库看一眼 `vps桥/池.py` 里的版本号，变了就把桥的几个文件全下下来，
...
| 配置项 | 说明 | 默认 |
|---|---|---|
| `auto_update` | `1` 开，`0` 关 | `1` |
| `update_minutes` | 多少分钟查一次 | `5` |
| `update_base` | 拉代码的仓库 raw 根地址 | 本仓库 main |

手动更新还是那条命令，和自动更新走同一套校验：
...
三格地区留空时，每批从 `随机国库` 里抽一个国家。现在只剩人工验过 dola 能正常用的 9 个：
日本、韩国、新加坡、泰国、越南、马来西亚、菲律宾、印尼、巴西。
其余地区提得到 IP 也会撞区域受限页，别往里加。
