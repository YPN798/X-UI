...
接口表 = (
    ...
    {"method": "POST", "path": "/api/sc/setup", "desc": "写入提取源并自动开跑",
     "body": {"provider": "auto", "key": "闪臣key", "code": "安全码",
              "go_key": "", "p24_token": ""}},
    {"method": "POST", "path": "/api/sc/white", "desc": "把本机或指定 IP 加进白名单（闪臣/1024）",
    ...
    return {
        ...
        "字段": {
            ...
            "可写": ["mode", "sticky", "fetch_url", "fetch_cmd", "fetch_scheme",
                    ...
                    "wall_port", "proxy_on", "sc_code", "go_pass", "p24_token", "p24_pass",
                    "随机国库"],
            "出厂随机国库": list(默随机国库),
            "国名": dict(国名表),
        },
    ...
    try:
        ...
        if 法 == "OPTIONS" and 路.startswith("/api"):
            ...
            步 = await 池子.一键开跑(
                ...
                str(数据.get("p24_token") or "").strip(),
                str(数据.get("p24_user") or "").strip(),
